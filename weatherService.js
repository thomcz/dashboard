...
function setWeather(forecastWeather, index) {
    ...
    document.getElementById(`forecast-weather${index}`).src = `http://openweathermap.org/img/wn/${icon}@2x.png`;
    document.getElementById(`forecast-weather-desc${index}`).innerHTML = forecastWeather.weather[0].description;
    document.getElementById(`forecast-weather-time${index}`).innerHTML = `${hour}:00`;
    document.getElementById(`forecast-weather-temp${index}`).innerHTML = Math.round(forecastWeather.main.temp) + '°';  
}
